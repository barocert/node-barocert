...
// 본인인증 검증
BaseService.addMethod(PasscertService.prototype, 'verifyIdentity', function (ClientCode, ReceiptID, PassIdentityVerify, success, error) {
  if (!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999, '이용기관코드가 입력되지 않았습니다.', error);
    return;
  }
  ...
  if (0 === Object.keys(PassIdentityVerify).length) {
    this._throwError(-99999999, '본인인증 검증 요청 정보가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassIdentityVerify.receiverHP || 0 === PassIdentityVerify.receiverHP.length) {
    this._throwError(-99999999, '수신자 휴대폰번호가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassIdentityVerify.receiverName || 0 === PassIdentityVerify.receiverName.length) {
    this._throwError(-99999999, '수신자 성명이 입력되지 않았습니다.', error);
    return;
  }

  var postData = this._stringify(PassIdentityVerify);
...
// 전자서명 검증
BaseService.addMethod(PasscertService.prototype, 'verifySign', function (ClientCode, ReceiptID, PassSignVerify, success, error) {
  if (!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999, '이용기관코드가 입력되지 않았습니다.', error);
    return;
  }
  ...
  if (0 === Object.keys(PassSignVerify).length) {
    this._throwError(-99999999, '전자서명 검증 요청 정보가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassSignVerify.receiverHP || 0 === PassSignVerify.receiverHP.length) {
    this._throwError(-99999999, '수신자 휴대폰번호가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassSignVerify.receiverName || 0 === PassSignVerify.receiverName.length) {
    this._throwError(-99999999, '수신자 성명이 입력되지 않았습니다.', error);
    return;
  }

  var postData = this._stringify(PassSignVerify);
...
// 출금동의 검증
BaseService.addMethod(PasscertService.prototype, 'verifyCMS', function (ClientCode, ReceiptID, PassCMSVerify, success, error) {
  if (!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999, '이용기관코드가 입력되지 않았습니다.', error);
    return;
  }
  ...
  if (0 === Object.keys(PassCMSVerify).length) {
    this._throwError(-99999999, '출금동의 검증 요청 정보가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassCMSVerify.receiverHP || 0 === PassCMSVerify.receiverHP.length) {
    this._throwError(-99999999, '수신자 휴대폰번호가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassCMSVerify.receiverName || 0 === PassCMSVerify.receiverName.length) {
    this._throwError(-99999999, '수신자 성명이 입력되지 않았습니다.', error);
    return;
  }

  var postData = this._stringify(PassCMSVerify);
...
// 간편로그인 검증
BaseService.addMethod(PasscertService.prototype, 'verifyLogin', function (ClientCode, ReceiptID, PassLoginVerify, success, error) {
  if (!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999, '이용기관코드가 입력되지 않았습니다.', error);
    return;
  }
  ...
  if (0 === Object.keys(PassLoginVerify).length) {
    this._throwError(-99999999, '간편로그인 검증 요청 정보가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassLoginVerify.receiverHP || 0 === PassLoginVerify.receiverHP.length) {
    this._throwError(-99999999, '수신자 휴대폰번호가 입력되지 않았습니다.', error);
    return;
  }
  if (!PassLoginVerify.receiverName || 0 === PassLoginVerify.receiverName.length) {
    this._throwError(-99999999, '수신자 성명이 입력되지 않았습니다.', error);
    return;
  }

  var postData = this._stringify(PassLoginVerify);
