...
function KakaocertService(config) {
    ...
    if(this.UseGAIP){
      this.ServiceURL = 'ga-barocert.linkhub.co.kr'; // TODO :: 나중에 바꿔야 함.
    } else if(this.UseStaticIP){
      this.ServiceURL = 'static-barocert.linkhub.co.kr';
    } else {
      this.ServiceURL = 'bc-api.linkhub.kr';
    }
...
KakaocertService.prototype._getToken = function (ClientCode, err) {
    ...
    if (expired) {
        newToken = linkhub.newToken(this.ServiceID, ClientCode, this._getScopes(), this.IPRestrictOnOff ? null : '*', this.UseStaticIP, this.UseGAIP);
...
KakaocertService.prototype._executeAction = function (options) {
    ...
    Token(function (token) {
      ...
      if(options.Method == 'POST') {
        var xDate = linkhub.getTime(_this.UseStaticIP, _this.UseGAIP);
        ...
        headers['x-bc-auth'] = _this._config.LinkID+' '+digest;
...
// 전자서명 요청(단건)
KakaocertService.addMethod(KakaocertService.prototype, 'requestESign', function(ClientCode, RequestESign, appUseYN, success, error){
...
// 전자서명 요청(다건)
KakaocertService.addMethod(KakaocertService.prototype, 'bulkRequestESign', function(ClientCode, RequestESign, appUseYN, success, error){
  if(!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999,'이용기관코드가 입력되지 않았습니다.',error);
    return;
  }
...
KakaocertService.addMethod(KakaocertService.prototype, 'getESignState', function(ClientCode, ReceiptID, success, error){
  ...
  this._executeAction({
    uri : '/KAKAO/ESign/Status/' + ClientCode + "/" + ReceiptID,
    ClientCode : ClientCode,
    Method : 'GET',
    success : function(response){
      if(success) success(response);
...
// 전자서명 상태확인(다건)
KakaocertService.addMethod(KakaocertService.prototype, 'getBulkESignState', function(ClientCode, ReceiptID, success, error){
  if(!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999,'이용기관코드가 입력되지 않았습니다.',error);
    return;
  }
  ...
  this._executeAction({
    uri : '/KAKAO/ESign/BulkStatus/' + ClientCode + "/" + ReceiptID,
    ClientCode : ClientCode,
    Method : 'GET',
    success : function(response){
      if(success) success(response);
...
// 전자서명 검증(다건)
KakaocertService.addMethod(KakaocertService.prototype, 'bulkVerifyESign', function(ClientCode, ReceiptID, success, error){
  if(!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999,'이용기관코드가 입력되지 않았습니다.',error);
    return;
  }
...
KakaocertService.addMethod(KakaocertService.prototype, 'getVerifyAuthState', function(ClientCode, ReceiptID, success, error){
  ...
  this._executeAction({
    uri : '/KAKAO/VerifyAuth/Status/' + ClientCode + "/" + ReceiptID,
    ClientCode : ClientCode,
    Method : 'GET',
    success : function(response){
      if(success) success(response);
...
KakaocertService.addMethod(KakaocertService.prototype, 'verifyAuth', function(ClientCode, ReceiptID, success, error){
  ...
  this._executeAction({
    uri : '/KAKAO/VerifyAuth/Verify/' + ReceiptID,
    ClientCode : ClientCode,
    Method : 'GET',
    success : function(response){
      if(success) success(response);
...
// 출금동의 요청
KakaocertService.addMethod(KakaocertService.prototype, 'requestCMS', function(ClientCode, RequestCMS, appUseYN, success, error){
  if(!ClientCode || 0 === ClientCode.length) {
    this._throwError(-99999999,'이용기관코드가 입력되지 않았습니다.',error);
    return;
  }
...
KakaocertService.addMethod(KakaocertService.prototype, 'getCMSState', function(ClientCode, ReceiptID, success, error){
  ...
  this._executeAction({
    uri : '/KAKAO/CMS/Status/' + ClientCode + '/' + ReceiptID,
    ClientCode : ClientCode,
    Method : 'GET',
    success : function(response){
      if(success) success(response);
